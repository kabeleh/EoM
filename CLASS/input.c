...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case Omega_scf:
      ...
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_tot]/(ba.H0*ba.H0)
        -1;//HVR
      // HVR ->
      //printf("  -- Output=%e   xguess=%e\n\n", output[i], ba.scf_parameters[ba.scf_tuning_index]);
      // <- HVR 
      break;
    case Omega_ini_dcdm:
    case omega_ini_dcdm:
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_scf:
      ...
      else if (ba.scf_tuning_index == 4){
        xguess[index_guess] = ba.scf_parameters[ba.scf_tuning_index];
        dxdy[index_guess] = 1.;
        //dxdy[index_guess] = 3*ba.H0*ba.H0/ba.Omega0_scf/(2*exp(ba.scf_parameters[2]*ba.scf_parameters[3]));//in CLASS units, rho_crit,0 = H0^2
        //dxdy[index_guess] = log(xguess[index_guess]);
        //printf("xguess = %g\nrho_scf = %g\ndxdy = %g\nH0 = %g\nOmega0_scf = %g\n",xguess[index_guess],ba.index_bg_rho_scf,dxdy[index_guess],ba.H0,ba.Omega0_scf);
      }
      else{
        /** Default: take the passed value as xguess and set dxdy to 1.
         *	y = Omega_scf, the target parameter
         *	xguess = the tuned parameter
         *	dxdy = derivative of xguess w.r.t. y
         **/
        xguess[index_guess] = ba.scf_parameters[ba.scf_tuning_index];
        dxdy[index_guess] = 0.01;
        printf("xguess = %g\n",xguess[index_guess]);
      }//KBL: 0
      break;
    case omega_ini_dcdm:
