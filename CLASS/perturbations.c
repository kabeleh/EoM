  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppt->has_source_delta_m == _TRUE_) {
          ...
          else delta_rho_m += ppw->pvecback[pba->index_bg_rho_cdm]*y[ppw->pv->index_pt_delta_cdm]; // contribution to delta rho_matter
      //KBL:0
        rho_m += ppw->pvecback[pba->index_bg_rho_cdm];
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
...
int perturb_derivs(double tau,
  ...
  /**KBL: Get the value of Q once, instead of calling the function multiple times in the CDM and SCF equations */
