...
int background_functions(
  ...
  double phi, phi_prime, scf_b, scf_c, phi_null, scf_C_pot, c_twiddl, has_tanh;//KBL
  ...
  /* cdm */ //KBL: 1
  c_twiddl	=	pba->scf_parameters[5];
  phi_null	=	pba->scf_parameters[3];
  has_tanh  = pba->scf_parameters[0];
  if (pba->has_cdm == _TRUE_) {
    if (pba->has_scf == _TRUE_) {
      if (fabs(has_tanh-1) < 0.00001) {
        pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3) * (1 - tanh(c_twiddl * (pvecback[pba->index_bg_phi_scf] - phi_null)))/2;
        ...
        if (pvecback[pba->index_bg_phi_scf] < phi_null) {
        pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3);
        rho_tot += pvecback[pba->index_bg_rho_cdm];
        p_tot += 0.;
        rho_m += pvecback[pba->index_bg_rho_cdm];
        //printf("1");
        //printf("CMD: phi<phi_0\n");
        }
        else if (pvecback[pba->index_bg_phi_scf] >= phi_null) {
          pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3) * exp(-c_twiddl * (pvecback[pba->index_bg_phi_scf] - phi_null));
          rho_tot += pvecback[pba->index_bg_rho_cdm];
          p_tot += 0.;
          rho_m += pvecback[pba->index_bg_rho_cdm];
          //printf("0");
        ...
        else printf("Error in CDM Definition in background.c\n");
      }
    }//KBL: 0
  	else {
    	pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3);
...
int background_solve(
  ...
  if (pba->background_verbose > 2) {
    ...
    if (pba->has_scf == _TRUE_){
      ...
      printf("     -> initial parameters: [tanh [1/0]\t,\tb\t,\tc\t,\tphi_0\t,\tC\t,\tc_twiddl,\tphi\t,\tphi'\t] = \n");//KBL
      printf("                            [");
...
int background_initial_conditions(
  ...
  // HVR ->
  // scf parameters:
  double scf_b, scf_c, scf_phi0, scf_C;
  // <- HVR
  ...
  */
  ...
  if(pba->has_scf == _TRUE_){
    ...
    if(pba->attractor_ic_scf == _TRUE_){
      ...
      scf_c = pba->scf_parameters[2];
      scf_phi0 = pba->scf_parameters[3];
      scf_C = pba->scf_parameters[4];
      ...
      class_test(pvecback_integration[pba->index_bi_phi_scf] > scf_phi0,
             pba->error_message,
             "Wrong initial condition: phi_ini > phi_0.");
      /**class_test(pvecback_integration[pba->index_bi_phi_scf] < 0,
             pba->error_message,
             "Wrong initial condition: phi_ini < 0");*/
             
      //~ printf("Omega_r = %g,   H0 = %g\n", pba->Omega0_g, pba->H0);
      //~ printf("log(x) -> x=%g\n", 4.*pba->Omega0_g*pba->H0*pba->H0/scf_b/scf_b - 2./3.*a*a*a*a);
      // <- HVR
    }    
    else{
    ...
    if (fabs(pba->scf_parameters[0]-1) < 0.00001) {
      ...
      if (pvecback[pba->index_bg_phi_scf] < pba->scf_parameters[3]) {
        dy[pba->index_bi_phi_prime_scf] = - y[pba->index_bi_a]*
          ...
          + y[pba->index_bi_a]*dV_scf(pba,y[pba->index_bi_phi_scf])) ;
          //printf("0");
          //printf("%g < %g\t",pvecback[pba->index_bg_phi_scf],pba->scf_parameters[3]);
        }
      else if (pvecback[pba->index_bg_phi_scf] >= pba->scf_parameters[3]) {
        dy[pba->index_bi_phi_prime_scf] = - y[pba->index_bi_a]*
          (2*pvecback[pba->index_bg_H]*y[pba->index_bi_phi_prime_scf]
          + y[pba->index_bi_a]*dV_scf(pba,y[pba->index_bi_phi_scf])) + pow(y[pba->index_bi_a],2)*pba->scf_parameters[5]*pvecback[pba->index_bg_rho_cdm];//pba->scf_parameters[5] points to c_twiddl
          //printf("1");
          //printf("%g > %g\t",pvecback[pba->index_bg_phi_scf],pba->scf_parameters[3]);
        }
...
double V_e_scf(struct background *pba,
  ...
  double scf_b      = pba->scf_parameters[1];
  double scf_c      = pba->scf_parameters[2];
  double phi_null   = pba->scf_parameters[3];
  
  return  exp(-scf_b*phi)*exp(phi_null*(scf_b-scf_c))+exp(-scf_c*phi);//V_e: exponential term of Pot V
}
...
double dV_e_scf(struct background *pba,
  ...
  return -scf_b*exp(-scf_b*phi)*exp(phi_null*(scf_b-scf_c))-scf_c*exp(-scf_c*phi);
}
...
double ddV_e_scf(struct background *pba,
  ...
  return pow(scf_b,2)*exp(-scf_b*phi)*exp(phi_null*(scf_b-scf_c))+pow(scf_c,2)*exp(-scf_c*phi);
}
...
double V_p_scf(
               struct background *pba,
               double phi) {
  double scf_C_pot      = pba->scf_parameters[4];
